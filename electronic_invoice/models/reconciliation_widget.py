...
class AccountReconciliation(models.AbstractModel):
    ...
    def _prepare_move_lines(self, move_lines, target_currency=False, target_date=False, recs_count=0):
        ...
        for line in move_lines:
            ...
            ret_line = {
                'id': line.id,
                # 'name': str(line.invoice_id.fe_Serie) + '-' + str(line.invoice_id.fe_DocNro) if line.invoice_id.fe_Serie else line.name and line.name != '/' and line.move_id.name + ': ' + line.name,
                'name': str(line.invoice_id.fe_Serie) + '-' + str(line.invoice_id.fe_DocNro) if line.invoice_id.fe_Serie else line.name and line.name != '/' and line.move_id.name + ': ' + line.name,
